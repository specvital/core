...
func TestStrategy_Parse(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// When
			file, err := s.Parse([]byte(tt.source), tt.filename)
...
func TestStrategy_Parse_Modifiers(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// When
			file, err := s.Parse([]byte(tt.source), "test.ts")
...
func TestStrategy_Parse_Location(t *testing.T) {
  ...
  it('test', () => {});
	...
	// When
	file, err := s.Parse([]byte(source), "user.test.ts")
...
func TestRegisterDefault(t *testing.T) {
	// NOTE: 글로벌 상태를 변경하므로 t.Parallel() 사용 불가
